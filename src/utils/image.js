...
/**
 * 压缩图片
 * @param file {File|String} 文件对象或者是图片的base64格式
 * @param options {Object} options.compress控制压缩参数
 * @return {Promise<any>}
 */
export function compress (file, options = {
  ...
  return new Promise(function (resolve, reject) {
    if (typeof file === 'string') { // base64
      let blob = dataURItoBlob(file)
      resolve({ file: null, blob, url: URL.createObjectURL(blob) })
    } else { // 从文件系统中选中
