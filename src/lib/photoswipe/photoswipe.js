/*
* 这是更改过源码后的photoswipe库
* patch single tap to close photoswipe (tap.js)
* fix android device can not call save image to device (_onDragStart)
*
...
export default function PhotoSwipe (template, UiClass, items, options) {
  ...
  var _animateProp = function (name, b, endProp, d, easingFn, onUpdate, onComplete) {
    var startAnimTime = _getCurrentTime(); var t
  ...
  var DIRECTION_CHECK_OFFSET = 10 // amount of pixels to drag to determine direction of swipe
  ...
  var _oldAndroidTouchEndTimeout
  ...
  var _convertTouchToPoint = function (touch, p) {
    p.x = touch.pageX
    p.y = touch.pageY
  ...
  var _onDragStart = function (e) {
    ...
    if (_preventDefaultEventBehaviour(e, true)) {
      // fix android device can not call save image to device
  ...
  _registerModule('Tap', {
    publicMethods: {
      ...
      onTapStart: function (touchList) {
        ...
        // patch single tap to close photoswipe
        tap_num++
        if (tap_num < 2) {
          setTimeout(function () {
            if (tap_num < 2 && !_isDragging && !_moved && !_isMultitouch && !_numAnimations) {
              self.close()
            }
            tap_num = 0
          }, 200)
  ...
  var _hashChangeTimeout
  
  var _hashAnimCheckTimeout
  
  var _hashChangedByScript
  
  var _hashChangedByHistory
